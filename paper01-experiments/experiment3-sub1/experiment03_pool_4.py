...
MAX_EPOCHS_LIMIT = 100     # 单个任务的最大迭代轮数，测定极限时使用更大的值
...
class RSInertiaRegulator:
    ...
    def record_epoch_state(self, tracker, current_epoch):
        # 1. 拿到本轮的矢量数据
    ...
    def get_batch_lr(self, g_train_batch, g_val_batch, val_loss_batch, current_epoch, scheduled_lr=None, w_inertia = 1.0):
        """
        ...
        if self.batch_print_counter % 50 == 0:
            print(f"      (B-Reg) S_b:{S_batch:.4f} [c_p:{c_p:.2f} c_p_raw:{c_p_raw:.2f} e2b_loss_p_ratio: {self.epoch2batch_anchor_p_loss/self.fixed_e2b_loss_p_base:.2f} | c_l:{c_l:.2f} c_l_raw:{c_l_raw:.2f} e2b_loss_l_ratio: {self.epoch2batch_anchor_l_loss/self.fixed_e2b_loss_l_base:.2f} | c_phi:{c_phi:.2f} c_phi_raw:{c_phi_raw:.2f}] "
                  f"| LR_rel:{eta_rel:.7f} | LR_thaw:{eta_thaw:.7f} | dt:{delta_t:.2f} | scheduled_lr:{scheduled_lr:.7f} | final_lr:{final_lr:.7f}")
...
def run_branch_shock(factory, name, fixed_D, fixed_E2B_LOSS_P_BASE=None, fixed_E2B_LOSS_L_BASE=None):
    ...
    for epoch in range(MAX_EPOCHS_LIMIT):
        is_noisy = (epoch >= MAX_EPOCHS_LIMIT // 2)
        if epoch == MAX_EPOCHS_LIMIT // 2: print(f"\n📢 [SHOCK] Injecting 50% Noise...")
        ...
        for x, y in train_loader:
            ...
            curr_lr, S_t, dt, omega = regulator.get_batch_lr(g_train_vec, g_train_vec, loss_pre.item(), epoch)
...
def run_branch_continual(factory, name, fixed_D, fixed_E2B_LOSS_P_BASE=None, fixed_E2B_LOSS_L_BASE=None):
    ...
    for task_id in tasks:
        ...
        for epoch in range(epochs_per_task):
            ...
            for x, y in train_dl:
                ...
                # --- 2. 调节决策 (使用当前感知) ---
                curr_lr, S_t, dt, omega = regulator.get_batch_lr(g_train_vec, g_train_vec, 2.3, abs_epoch) # 基准势能
    ...
    return history
def run_branch_baseline(factory, name, fixed_D, test_type="continual", fixed_E2B_LOSS_P_BASE=None, fixed_E2B_LOSS_L_BASE=None):
    """
...
def run_experiment_3():
    ...
    with open('exp03_final_comparison_composed_limit_results1_pool4.json', 'w') as f:
        json.dump(final_results, f, indent=4)
    ...
    with open('exp03_final_comparison_composed_limit_results2_pool4.json', 'w') as f:
        json.dump(final_results, f, indent=4)
